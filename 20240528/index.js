...
(function() {
    ...
    jQuery(document).ready(function($) {
		...
		class GHN_MULTICHECK{
			...
			initHTML(){
				$('.header-container .row .col-md-8').removeClass('col-md-8').addClass('col-md-10');
				$('#root .header').append(`<div class="modal fade bd-example-modal-lg" tabindex="-1" role="dialog" aria-labelledby="myLargeModalLabel" id="myModal" aria-hidden="true">
    ...
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="submit" form="formMultiSearch" class="btn btn-primary">Tìm kiếm</button>
				...
				$('.Flex.content').prepend(`<div class="row" style="background-color:#fff;"><div class="container"><table class="table" id="resultMulti" style="display:none">
  <thead>
    <tr>
  ...
  <tbody></tbody></table></div></div>`)
				...
				$('div.header .header-center').append(<button data-toggle="modal" data-target=".bd-example-modal-lg" class="button-search block-center btn btn-primary">TÌM KIẾM NÂNG CAO</span></button>)
			}
			initEvent(){
				let self = this;
				$('#formMultiSearch').submit(function(event){
					...
					//console.log(data);
					let ids = data.find(x=>x.name=="ids").value.trim().split("\n");
					//console.log(ids);
					let filter = data.filter(x=>x.name=="filter").map(x=>x.value);
                    //alert(filter.toString());
					//console.log(filter);
					$('#myModal').modal("hide");
					$('#resultMulti thead tr').html(<th scope="col" data-value="id">#</th>)
					filter.forEach(item=>{
                        //alert(item.toString());
						let itemFilter = self.filter.find(x=>x.name==item);
                        //console.log(itemFilter);
						if(itemFilter){
							$('#resultMulti thead tr').append(<td scope="col" data-value="${itemFilter.name}">${itemFilter.value}</td>)
						}
					})
...
//callog
							...
							if(json.code==200){
								...
								let UNPACKED_AT_LASTMILE = json.data.tracking_logs.filter(x=>x.action_code=="RECEIVED_AT_LASTMILE").slice(-1);
...
//XUẤT TIME TỚI KHO
                                    ...
                                    if(filter.findIndex(x=>x=="time")!=-1&&datastoring1[0]){
                                        ...
                                        if(UNPACKED_AT_LASTMILE.length>0){
                                            html +=`<td>${moment(UNPACKED_AT_LASTMILE[0].action_at).fromNow()}</td>`
                                        }else{
...
//xuất lần giao
								if(filter.findIndex(x=>x=="langiao")!=-1){
									...
									if(langiao>2) html += <td style="background-color: red">${langiao} lần</td>;
...
//xuất lý do + tên NV
                          ...
                          //ALL DATA CHO NAY
                                     ...
                                     if(trangthaicuoi.status=="waiting_to_return_to_storing"|trangthaicuoi.status=="return"|trangthaicuoi.status=="waiting_to_return"){
                                         ...
                                         if(nhanviencuoicunggiao.length>0){
                                             ...
                                             if(nhanviencuoicunggiao[0].reason!=null){
                                                 ...
                                                 html += <td>${nhanviencuoicunggiao[0].reason.split("-")[0]} </td>;
                                             }else{
                                     ...
                                     if(trangthaicuoi.action_code=="ADD_DELIVERY_TRIP" && trangthaicuoi.status=="storing"){
                                         tenNV+=1;
                                     ...
                                     //sang sang giao hang
                                     if(trangthaikecuoi.action_code=="RECEIVED_AT_LASTMILE" && trangthaicuoi.status=="storing"){
                                     ...
                                     if((trangthaikecuoi.action_code=="PACKED_TO_SORTING" && trangthaikecuoi.status=="storing")|(trangthaikecuoi.action_code=="SCAN_TO_STORING" && trangthaikecuoi.status=="picked_to_storing")){
                                         if(tenNV==0){
                                         tenNV+=1;
                                         //alert("DA VO");
                                         //console.dir(trangthaicuoi);
                                     ...
                                     {
                                         //alert(tenNV);
                                         if(tenNV==0){html +=`<td></td>`;html +=`<td></td>`;html +=`<td></td>`;}
...
//Xuất time cập nhật - chỉ hiển thị khi đơn GTC/Giao that bai
                                if(filter.findIndex(x=>x=='timeUpdate')!=-1){
                                    ...
                                    if(trangthaicuoi.status=="delivered"){
                                        ...
                                        //alert("vo");
                                        let delivered=json.data.tracking_logs.filter(x=>x.status=="delivered").slice(-1);
                                        html += <td>${moment(delivered[0].action_at).format('DD/MM/YYYY, HH:mm')} </td>;
                                    }
                                    if(trangthaicuoi.status=="delivery_fail"|trangthaicuoi.status=="delivery_fail_to_storing"){
                                        timeUP+=1;
                                        ...
                                        if(Array.isArray(deliveryfail) && deliveryfail.length){
                                            //alert("voo");
                                            html += <td>${moment(deliveryfail[0].action_at).format('DD/MM/YYYY, HH:mm')} </td>;
                                        }
                                    ...
                                    if(trangthaicuoi.status=="returned"){
                                        timeUP+=1;
                                        let returned=json.data.tracking_logs.filter(x=>x.status=="returned").slice(-1);
                                        html += <td>${moment(returned[0].action_at).format('DD/MM/YYYY, HH:mm')} </td>;
                                    }
                                    if(trangthaicuoi.status=="picked"|trangthaicuoi.status=="picked_to_storing"|(trangthaikecuoi.action_code=="PACKED_TO_SORTING"&& trangthaikecuoi.status=="storing")|(trangthaicuoi.status=="storing" && trangthaicuoi.action_code=="RECEIVED_AT_LASTMILE")){
                                        timeUP+=1;
                                        let picked=json.data.tracking_logs.filter(x=>x.status=="picked").slice(-1);
                                        //alert("da vo");
                                        //console.dir(picked);
                                        if(picked.length==0){
                                            let DELIVER_FAILED=json.data.tracking_logs.filter(x=>x.action_code=="DELIVER_FAILED");
                                            if(DELIVER_FAILED.length>0){
                                                html += <td>${moment(DELIVER_FAILED[0].action_at).format('DD/MM/YYYY, HH:mm')} </td>;
                                                html +=`<td></td>`;
                                            }
                                        }else{
                                        html += <td>${moment(picked[0].action_at).format('DD/MM/YYYY, HH:mm')} </td>;}
                                    }
                                    if(trangthaicuoi.status=="waiting_to_return_to_storing"|trangthaicuoi.status=="waiting_to_return"|trangthaicuoi.status=="return"|trangthaicuoi.status=="return_transporting"){
                                        timeUP+=1;
                                        let delivery_fail=json.data.tracking_logs.filter(x=>x.status=="delivery_fail").slice(-1);
                                        //alert(delivery_fail.length);
                                        if(delivery_fail.length==0){
                                            delivery_fail=json.data.tracking_logs.filter(x=>x.status=="return");
                                        }else{
                                        console.dir(delivery_fail);
                                        if(delivery_fail.indexOf("action_at"))
                                        {html += <td>${moment(delivery_fail[0].action_at).format('DD/MM/YYYY, HH:mm')} </td>;}
                                        }
                                    }
                                    if(trangthaicuoi.status=="pick_fail"){
                                        timeUP+=1;
                                        let pick_fail=json.data.tracking_logs.filter(x=>x.status=="pick_fail").slice(-1);
                                        html += <td>${moment(pick_fail[0].action_at).format('DD/MM/YYYY, HH:mm')} </td>;
                                    }
                                    //"returned"
                                    //delivering
                                    if(timeUP==0){
...
//Time eps chuyen di
                                if(filter.findIndex(x=>x=="timeEp")!=-1){
                                    ...
                                    let khogannhat=json.data.tracking_logs.slice(-1);
                                    //console.dir(khogannhat);
                                    let EpGiao = json.data.tracking_logs.filter(x=>x.action_code=="START_DELIVERY_TRIP").slice(-1);
                                    ...
                                    let COLLECT_DELIVERING_MONEY=json.data.tracking_logs.find(x=>x.action_code=="COLLECT_DELIVERING_MONEY");
                                    //console.dir(EpGiao);
                                    let EpLay = json.data.tracking_logs.filter(x=>x.action_code=="START_PICKING_TRIP").slice(-1);
                                    //console.dir(EpLay);
                                    let EpTra = json.data.tracking_logs.filter(x=>x.action_code=="START_RETURN_TRIP").slice(-1);
                                    console.dir(EpTra);

                                    //Chỉ xuất khi trạng thái cuối ở kho kontum
                                    if(khogannhat[0].location.warehouse_id==20394000|khogannhat[0].location.warehouse_id==20375000|khogannhat[0].location.warehouse_id==2310|khogannhat[0].location.warehouse_id==20257000){
                                        if(CANCEL_COLLECT_DELIVERING_MONEY){
                                            timeUP+=1;
                                            //alert("1");
                                             html += <td>Đã đc ép giao ${moment(CANCEL_COLLECT_DELIVERING_MONEY.action_at).format('DD/MM/YYYY, HH:mm')}</td>
                                        }
                                        if(COLLECT_DELIVERING_MONEY){
                                            //alert("1");
                                             html += <td>Đã đc ép giao ${moment(COLLECT_DELIVERING_MONEY.action_at).format('DD/MM/YYYY, HH:mm')}</td>
                                        }
                                        ...
                                        if(EpGiao.length!=0){
                                            ...
                                            if(EpGiao[0].location.warehouse_id==khogannhat[0].location.warehouse_id){
                                                console.dir(EpGiao);
                                                html += <td>Đã đc ép giao ${moment(EpGiao[0].action_at).format('DD/MM/YYYY, HH:mm')}</td>;
                                        ...
                                        if(EpLay.length!=0){
                                            ...
                                            if(EpLay[0].location.warehouse_id==khogannhat[0].location.warehouse_id){
                                                html += <td>Đã đc ép lấy ${moment(EpLay[0].action_at).format('DD/MM/YYYY, HH:mm')}</td>;

                                            }
                                        }
                                        if(EpTra.length!=0){
                                            timeUP+=1;
                                            if(EpTra[0].location.warehouse_id==khogannhat[0].location.warehouse_id){
                                                html += <td>Đã đc ép trả ${moment(EpTra[0].action_at).format('DD/MM/YYYY, HH:mm')}</td>;
...
//Xuất time tạo đơn
                                if(filter.findIndex(x=>x=="timeCreate")!=-1){
                                    ...
                                    if(timeCreate){
                                         html += <td>Đã đc tạo lúc ${moment(timeCreate.action_at).format('DD/MM/YYYY, HH:mm')}</td>;
                                    }else{
                                        html+='<td></td>';
                                    }
...
//Xuất time tạo LC
                                if(filter.findIndex(x=>x=="transferToTruck")!=-1){
                                    ...
                                    if(timeTransfer){
                                         html += <td>${moment(timeTransfer.action_at).format('DD/MM/YYYY, HH:mm')}</td>;
                                    }else{
                                        let timeTransporting=json.data.tracking_logs.find(x=>x.action_code=="TRANSPORTING");
                                        if(timeTransporting){
                                            html += <td>${moment(timeTransporting.action_at).format('DD/MM/YYYY, HH:mm')}</td>;
                                        }else{
                                            html+='<td></td>';
                                        }
                                    }
...
//Xuất NV Lấy
                                if(filter.findIndex(x=>x=="NVLay")!=-1){
                                    //alert("voo");
                                    let NVLAY=json.data.tracking_logs.filter(x=>x.action_code=="PICKED_IN_TRIP").slice(-1);
                                    //console.log(NVLAY);
                                    if(NVLAY.length!=0){
...
//Xuất SME-KA
                                if(filter.findIndex(x=>x=="SMEORKA")!=-1){
                                    //alert("voo");
                                     let SME=["Shopee","Lazada Market Place","tiki_c2c","Tiki.vn"];
                                     let ready_to_pick = json.data.tracking_logs.find(x=>x.status=="ready_to_pick");
...
//xuất lần lấy
								if(filter.findIndex(x=>x=="lanlay")!=-1){
									let lanlay = json.data.tracking_logs.filter(x=>x.status=="picking").length;
                                    //alert(lanlay);

									if(lanlay>3) html += <td style="background-color: red">${lanlay} lần</td>;
...
//Xuất lịch sử giao hàng
                                if(filter.findIndex(x=>x=="lichsu")!=-1){
                                    ...
                                    if(nhanviengiaocuoi.length>0){
                                        ...
                                        $.each(nhanviencuoicunggiao2,function(index,value){
                                            html+=`<p><b>Lần  ${index+1}:</b>${value.reason.split("-")[0].toString()}`;
                                        });
                                        html+='</p></td>';
...
//SDT NVXL Nhận tại kho
                                if(filter.findIndex(x=>x=="sdtNVXL")!=-1){
                                    let NVXL=json.data.tracking_logs.filter(x=>x.action_code=="ADD_DELIVERY_TRIP").slice(-1);
                                    let NVXL2=json.data.tracking_logs.filter(x=>x.action_code=="RECEIVED_AT_LASTMILE").slice(-1);
                                    console.log(NVXL);
                                    if(NVXL.length!=0){
